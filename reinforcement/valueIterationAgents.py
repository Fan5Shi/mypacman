...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for tvalue in tvalues:
            nstate, prob = tvalue
            rvalue = self.mdp.getReward(state, action, nstate)
            tpvalue += prob * (rvalue + gamma * self.getValue(nstate))
        return tpvalue
    ...
    def runValueIteration(self):
        ...
        for state in self.mdp.getStates():
            ...
            for action in self.mdp.getPossibleActions(state):
                temps = self.mdp.getTransitionStatesAndProbs(state, action)
                for nextstate, prob in temps:
                    temp1 = predecessors[nextstate]
                    temp1.add(state)
                    predecessors[nextstate] = temp1
        ...
        for i in range(self.iterations):
            ...
            for pre in predecessors[state]:
                if self.mdp.isTerminal(pre):
                    continue
                differ = abs(self.values[pre] - max([self.getQValue(pre, action) for action in self.mdp.getPossibleActions(pre)]))
                if differ > self.theta:
                    queue.update(pre, -differ)
